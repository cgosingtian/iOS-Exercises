...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSDictionary *course = _courses[indexPath.row];
    NSArray *upcoming = course[@"upcoming"];
    if ([[cell class] isSubclassOfClass:[KLBUpcomingCoursesCell class]])
    {
        KLBUpcomingCoursesCell *upCell = (KLBUpcomingCoursesCell *)cell;
        if (upCell == nil) NSLog(@"NIL");
        upCell.courseLabel.text = course[@"title"];
        upCell.textLabel.text =course[@"title"];
        upCell.upcomingLabel.text = [NSString stringWithFormat:@"%@",upcoming[0][@"start_date"]];
        NSLog(@"%@, starting next at %@",upCell.courseLabel.text,upCell.upcomingLabel.text);
    }
    else NSLog(@"%@",[[cell textLabel]text]);
}
...
//           forCellReuseIdentifier:@"KLBUpcomingCoursesCell"];
    ...
    for (UINib *nib in [NSBundle mainBundle])
    {
        NSLog(@"nib %@", [nib description]);
    }
}
